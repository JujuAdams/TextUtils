...
function StringSplit_2(_string)
{
    ...
    //Empty string
    if (_string == "") return []; //_scope.__result;
    
    var _outArray = [undefined];
    global.stringSplit = _outArray;
    ...
    if (_firstSplit[0] != "")
    {
        string_foreach(_firstSplit[0], function(_character, _position)
        {
            array_push(global.stringSplit, ord(_character), undefined);
        });
    }
    ...
    repeat(array_length(_firstSplit)-1)
    {
        ...
        if (_escapeNext)
        {
            ...
            string_foreach(_substring, function(_character, _position)
            {
                array_push(global.stringSplit, ord(_character), undefined);
            });
        ...
        else
        {
            ...
            if (array_length(_secondSplit) == 1)
            {
                ...
                string_foreach(_secondSplit[0], function(_character, _position)
                {
                    array_push(global.stringSplit, ord(_character), undefined);
                });
            ...
            else
            {
                ...
                array_push(_tagArray, _tag);
                
                if (_secondSplit[1] != "")
                {
                    string_foreach(_secondSplit[1], function(_character, _position)
                    {
                        array_push(global.stringSplit, ord(_character), undefined);
                    });
